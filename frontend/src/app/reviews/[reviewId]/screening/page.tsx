...
export default async function ScreeningOperationsPage({ params, searchParams }: Props) {
    ...
    return (
      <main className="mx-auto min-h-screen w-full max-w-7xl px-6 py-8 sm:px-10 lg:px-14">
        <section className="rounded-2xl border border-amber-200 bg-amber-50 p-6 text-amber-900" role="alert">
          <h1 className="text-2xl font-semibold">Screening operations are unavailable</h1>
          <p className="mt-2 text-sm">The queue could not be loaded safely. No screening state was changed.</p>
        </section>
      </main>
    );
  }

  return <ReviewOperationsPanel focus="screening" reviewId={reviewId} query={query} workspace={result.workspace} />;
}
