...
from backend.app.ai.certainty_service import AICertaintyProposalView, AICertaintyService
...
router = APIRouter(prefix="/ai/certainty", tags=["ai-certainty"])


class CertaintySourceRequest(BaseModel):
    document_id: UUID
    document_role: str = Field(default="PRIMARY_FULL_TEXT", min_length=1, max_length=50)


class CertaintyReadinessRequest(BaseModel):
    assessment_id: UUID
    documents: list[CertaintySourceRequest] = Field(min_length=1, max_length=8)


class CertaintyBatchItemRequest(CertaintyReadinessRequest):
    pass


class CertaintyBatchRequest(BaseModel):
    items: list[CertaintyBatchItemRequest] = Field(min_length=1, max_length=100)
...
class CertaintyPolicyRequest(BaseModel):
    maximum_batch_size: int = Field(default=20, ge=1, le=100)


class CertaintyReviewRequest(BaseModel):
    action: AICertaintyReviewAction
    canonical_action: str | None = Field(default=None, max_length=50)
    human_payload: dict[str, Any] | None = None
    reason: str | None = Field(default=None, max_length=4_000)


class CertaintyDatasetRequest(BaseModel):
...
class CertaintyErrorRequest(BaseModel):
    case_key: str = Field(min_length=1, max_length=160)
    category: AICertaintyErrorCategory
    note: str | None = Field(default=None, max_length=4_000)


def _service(session: DbSessionDependency) -> AICertaintyService:
    ...
    certainty_repository = SqlAlchemyCertaintyRepository(session)
    certainty_service = CertaintyService(
        certainty_repository,
        outcomes,
        analyses,
        analysis_service,
    ...
    return AICertaintyService(
        SqlAlchemyAICertaintyRepository(session),
        ai_repository,
        certainty_repository,
...
def _proposal(item: AICertaintyProposalView) -> dict[str, Any]:
    return {
        "assessment_id": item.assessment_id,
        "outcome_version_id": item.outcome_version_id,
        "framework_version_id": item.framework_version_id,
...
async def create_policy(
    payload: CertaintyPolicyRequest,
...
async def readiness(
    payload: CertaintyReadinessRequest,
    actor: ActorContextDependency,
    session: DbSessionDependency,
    review_id: Annotated[UUID, Path()],
) -> dict[str, Any]:
...
async def review_proposal(
    payload: CertaintyReviewRequest,
    actor: ActorContextDependency,
    session: DbSessionDependency,
    proposal_id: Annotated[UUID, Path()],
    review_id: Annotated[UUID, Path()],
...
async def create_dataset(
    payload: CertaintyDatasetRequest,
    actor: ActorContextDependency,
    session: DbSessionDependency,
    review_id: Annotated[UUID, Path()],
) -> dict[str, Any]:
...
async def list_datasets(
    ...
    return [
        _dataset(item) for item in await _service(session).list_datasets(actor, review_id=review_id)
    ]
