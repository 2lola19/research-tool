...
- Backend: Ruff lint and format checks, strict mypy, and pytest pass: 111 tests, 94.84% coverage (configured threshold: 85%).
- Frontend: ESLint, TypeScript, Vitest (4 tests), and Next.js production build pass; no frontend code changed in Phase 11.
- Alembic is linear through `20260810_0011`. A temporary SQLite database upgrades from foundation through the document milestone and downgrades to base.
...
- Next phase: extraction schemas and verified manual/automated extraction design, followed by a separately reviewed implementation milestone.
...
See [ADR-012](adr/ADR-012-concurrent-sequential-allocation.md), [ADR-013](adr/ADR-013-document-processing-and-grobid-adapter.md), [API_REQUIREMENTS.md](API_REQUIREMENTS.md), and [OPEN_SOURCE_COMPONENTS.md](OPEN_SOURCE_COMPONENTS.md).
