...
## Phase 28 outcome/effect-estimate harmonization assistance

- Implemented the governed `OUTCOME_MAPPING_SUGGESTION` task over the existing `OutcomeService`. Verified extraction values, immutable outcome versions, Study-linked processed Documents, parser/chunk snapshots, allowed references, evidence quotes, and input hashes are pinned.
